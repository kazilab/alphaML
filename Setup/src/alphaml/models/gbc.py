...
from ..utils.utils import plot_roc_auc_acc, sample_weight_
...
def model_(data,
    ...
    model.fit(data, data_labels, sample_weight=sample_weight_(data_labels))
    ...
    for max_iter_space in max_iter_spaces:
        ...
        rf.fit(data, data_labels, sample_weight=sample_weight_(data_labels))
...
def roc_auc(labeled_data_s,
    ...
    for train, test in kf.split(xx):
        ...
        clf_roc.fit(x_train, y_train, sample_weight=sample_weight_(y_train))
...
def optuna_(data,
    ...
    def objective(trial):
        ...
        for train_index, test_index in kf.split(xx):
            ...
            clf_opti = Classifier(**params_, verbose=0)
            clf_opti.fit(x_train, y_train, sample_weight=sample_weight_(y_train))
...
def bayes_(data,
    ...
    opt = BayesSearchCV(Classifier(verbose=0,
                                   random_state=random_state
                                   ),
                        search_spaces=space,
...
def grid_(data,
    ...
    opt = GridSearchCV(Classifier(verbose=0,
                                  random_state=random_state
                                  ),
                       param_grid=space,
                       scoring=make_scorer(kappa_mcc_error, greater_is_better=False),
