...
def model_(data,
    ...
    model = Classifier(**parameters,
                       estimator=ExtraTreeClassifier(criterion="log_loss", max_depth=4, class_weight='balanced')
                       )
    ...
    max_iter = int(parameters['n_estimators'])
    parm_del_max_iter = parameters.copy()
    del parm_del_max_iter['n_estimators']
    ...
    for max_iter_space in max_iter_spaces:
        rf = Classifier(n_estimators=max_iter_space,
                        **parm_del_max_iter,
                        estimator=ExtraTreeClassifier(criterion="log_loss", max_depth=4, class_weight='balanced')
                        )
...
def optuna_(data,
    ...
    def objective(trial):
        learning_rate = trial.suggest_categorical("learning_rate", [0.005, 0.01, 0.05, 0.1, 0.5])
        ...
        for train_index, test_index in kf.split(xx):
            ...
            clf_opti = Classifier(**params_, estimator=ExtraTreeClassifier(criterion="log_loss",
                                                                           max_depth=4,
            ...
            # print('cv_score: ', cs)kappa_mcc_error(y_valid, clf_opti.predict(x_valid))
...
def bayes_(data,
    ...
    space = {'learning_rate': Categorical([0.005, 0.01, 0.05, 0.1, 0.5]),
             'n_estimators':  Categorical([100, 300])
             }
    opt = BayesSearchCV(Classifier(estimator=ExtraTreeClassifier(criterion="log_loss",
                                                                 max_depth=4,
                                                                 class_weight='balanced'
                                                                 ),
                                   random_state=random_state
    ...
    opt.fit(data, data_labels, sample_weight=sample_weight_(data_labels))
...
def grid_(data,
    ...
    space = {'learning_rate': [0.005, 0.01, 0.05, 0.1, 0.5],
             'n_estimators': [100, 125, 150, 175, 200, 225, 250, 275]
             }

    opt = GridSearchCV(Classifier(estimator=ExtraTreeClassifier(criterion="log_loss",
                                                                max_depth=4,
                                                                class_weight='balanced'
                                                                ),
                                  random_state=random_state
